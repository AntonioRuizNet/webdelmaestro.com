// lib/db.js
import pg from "pg";
import { getStaticArticleBySlug, getStaticArticlesForSitemap, searchStaticArticles } from "@/data/staticArticles";
import { mergePostsWithDbPriority } from "@/lib/staticArticles/mergePosts";
const { Pool } = pg;
...
export async function getPostBySlug(slug) {
  ...
  const row = rows[0];
  if (!row) return getStaticArticleBySlug(slug);
...
export async function getAllPostsForSitemap() {
  ...
  const usedSlugs = new Set(dbPosts.map((post) => post.slug));
  const staticPosts = getStaticArticlesForSitemap().filter((post) => !usedSlugs.has(post.slug));

  return [...dbPosts, ...staticPosts];
}
...
export async function searchPosts({ term = "", column = "title", limit = 12, random = false, exclude = [] }) {
  ...
  const staticRows = searchStaticArticles({
    term,
    column: col,
    limit,
    random: false,
  ...
  return mergePostsWithDbPriority(rows, staticRows, limit, random);
}
