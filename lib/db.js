...
export async function getAllPostsForSitemap() {
  ...
  // devolvemos los datos tal cual; convertimos fechas a string ISO en el sitemap
  return rows.map((row) => ({
    slug: row.slug,
...
export async function searchPosts({
  ...
  const allowedColumns = ["title", "slug", "body", "excerpt", "url", "search"];
  ...
  if (term) {
    if (col === "search") {
      // 🔍 BÚSQUEDA EN title + meta_description
      rows = await sql`
