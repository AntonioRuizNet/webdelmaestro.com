...
export function cleanHtml(html, excerpt) {
  ...
  // 2️⃣ bis – Eliminar div.entry-thumb (y en general cualquier .entry-thumb)
  document.querySelectorAll("div.entry-thumb").forEach((el) => el.remove());
  // o, si quieres eliminar cualquier tag con esa clase:
  // document.querySelectorAll(".entry-thumb").forEach((el) => el.remove());
